...
local FEAR_WARD_SPELL_ID = addon.GetFearWardSpellID and addon:GetFearWardSpellID() or 6346
local FEAR_WARD_SPELL_NAME = addon.GetFearWardSpellName and addon:GetFearWardSpellName() or "Fear Ward"
...
local function SaveFramePosition(frame, key)
    ...
    db.positions[key].relativePoint = relativePoint
    db.positions[key].x = x
    db.positions[key].y = y
    db.positions[key].width = frame:GetWidth()
    db.positions[key].height = frame:GetHeight()
    db.positions[key].scale = frame:GetScale()
end

local function RestoreFramePosition(frame, key, defaultPoint, defaultRelativePoint, defaultX, defaultY)
    local db = EnsureDB()
    ...
    if saved then
        frame:SetPoint(
            ...
            UIParent,
            saved.relativePoint or defaultRelativePoint,
            saved.x or defaultX,
            saved.y or defaultY
        )
...
local function AddResizeCorners(targetFrame, key, minW, minH, maxW, maxH, onResizeDone)
    targetFrame:SetResizable(true)

    if targetFrame.SetResizeBounds then
        targetFrame:SetResizeBounds(minW, minH, maxW, maxH)
    else
        targetFrame:SetMinResize(minW, minH)
        targetFrame:SetMaxResize(maxW, maxH)
    end
    ...
    for _, data in ipairs(corners) do
        ...
        grip:SetScript("OnMouseUp", function()
            ...
            SaveFramePosition(targetFrame, key)
...
RestoreFramePosition(alertFrame, "alert", "TOP", "TOP", 0, -180)
...
local function UpdateAlertFont()
    ...
    local size = math.max(18, math.min(44, math.floor(height * 0.45)))
    alertFrame.text:SetFont(STANDARD_TEXT_FONT, size, "OUTLINE")
end

AddResizeCorners(alertFrame, "alert", 260, 50, 1200, 220, UpdateAlertFont)
UpdateAlertFont()
...
local function ShowFWCAlert(message)
    ...
    alertTestMode = false
    alertHideGeneration = alertHideGeneration + 1
    local myGeneration = alertHideGeneration

    alertFrame.text:SetText(message)
    alertFrame:Show()

    C_Timer.After(3, function()
        if myGeneration == alertHideGeneration and not alertTestMode then
            alertFrame:Hide()
        end
    end)
end
...
SlashCmdList["FWCALERT"] = function()
    ...
    print("|cff00ff00Shift + right-drag a corner to resize.|r")
...
RestoreFramePosition(frame, "main", "CENTER", "CENTER", 0, 0)
MakeMovable(frame, "main")
AddResizeCorners(frame, "main", 120, 40, 400, 120)
...
RestoreFramePosition(popout, "popout", "CENTER", "CENTER", 0, -130)
MakeMovable(popout, "popout")
AddResizeCorners(popout, "popout", 190, 120, 520, 650)
...
local function ClearPopout()
    ...
    for _, object in ipairs(popoutObjects) do
        object:Hide()
...
local function AddText(text, x, y, template)
    local fontString = popout:CreateFontString(nil, "OVERLAY", template or "GameFontNormal")
    fontString:SetPoint("TOPLEFT", popout, "TOPLEFT", x, y)
    fontString:SetText(text)
    table.insert(popoutObjects, fontString)
    return fontString
end
...
local function AddAssignedGroupAlertTargets(targets)
    for _, groupNumber in ipairs(GetMyGroups()) do
...
local function AddMainTankAlertTargets(targets)
    ...
    for i = 1, 40 do
        ...
        if name and UnitExists(unit) then
            ...
            if isMainTank or raidRole == "MAINTANK" or assignedRole == "TANK" then
...
local function CreateFearWardButton(member, x, y, width, labelText)
    local button = CreateFrame("Button", nil, popout, "SecureActionButtonTemplate")
    button:SetSize(width, 16)
    button:SetPoint("TOPLEFT", popout, "TOPLEFT", x, y)

    button:SetAttribute("type", "spell")
    button:SetAttribute("spell", FEAR_WARD_SPELL_NAME or FEAR_WARD_SPELL_ID)
    ...
    table.insert(popoutObjects, button)
    return button
end

local function AddPlayerRow(member, x, y)
    local nameButton = CreateFearWardButton(member, x, y, 86, member.name)
    local statusButton = CreateFearWardButton(member, x + 112, y, 72, "...")
...
function addon:RefreshPopoutStatusOnly()
    for _, row in ipairs(playerRows) do
        ...
        local alpha = inRange and 1 or 0.8
...
function addon:BuildPopout()
    if InCombatLockdown() then
        print("|cffff0000FWC: Cannot rebuild secure buttons during combat. Open before pull.|r")
        return
    end
    ...
    if #groups == 0 then
        ...
        SaveFramePosition(popout, "popout")
        return
    end
    ...
    local neededHeight = math.max(120, math.min(70 + (line * 19), 650))
...
local function TogglePopout()
    ...
    if InCombatLockdown() and #playerRows == 0 then
        print("|cffff0000FWC: Open the popout before combat to enable click-casting.|r")
        return
    end
...
function addon:ToggleUI()
    if frame:IsShown() then
        if InCombatLockdown() then
            print("|cffffff00FWC: Cannot safely close secure frames during combat.|r")
            return
        end
...
eventFrame:SetScript("OnEvent", function(_, event)
    ...
    if not InCombatLockdown() and popout:IsShown() then
        addon:BuildPopout()
    end
